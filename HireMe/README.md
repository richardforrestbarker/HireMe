...
Since the graph is a rolling graph showing only the last year of contributions, the words "slide" across the graph over time, and then repeat.
This will execute via a GitHub Action on a schedule, but can also be run locally with Python 3 and Git installed.
